...
% constraints on x-states
grc_con = 0.2; % generation rate constraint
tie_con = 0.2; % tie-line constraint
other_x = inf;    % other states constraints
...
figure
subplot(3,1,1)
stairs(plt_t, xx(1,:),'k','LineWidth',2)
ylabel('\Delta f_1 [Hz]')
xlabel('Time [s]')
grid on
...
subplot(3,1,2)
stairs(plt_t, xx(4,:),'k','LineWidth',2)
ylabel('\Delta P_{tie,12} [pu]')
xlabel('Time [s]')
grid on
fontname(gcf,"Garamond")
fontsize(gcf,20,"pixels")
...
subplot(3,1,3)
stairs(plt_t, xx(5,:),'k','LineWidth',2)
ylabel('\Delta f_2 [Hz]')
xlabel('Time [s]')
grid on
fontname(gcf,"Garamond")
